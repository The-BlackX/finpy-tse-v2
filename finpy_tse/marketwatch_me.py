...
def get_marketwatch_me(output="dataframe", filename="MarketWatch", add_timestamp=False, save_path=None):
    ...
    # Process market watch data (section @2)
    ...
    # Process order book data (section @3, depth 1 only)
    ...
    # Merge market watch and order book data
    ...
    # Add client type data (section @7)
    try:
        Client_df = pd.DataFrame((main_text.split('@')[7]).split(';'))
        ...
        # If section @7 is missing, fill with NaN
        for col in ['R_Buy_C', 'Co_Buy_C', 'R_Buy_Vol', 'Co_Buy_Vol', 'R_Sell_C', 'Co_Sell_C', 'R_Sell_Vol', 'Co_Sell_Vol']:
            final_df[col] = pd.NA
    ...
    # Add Download column with current Jalali timestamp
    final_df['Download'] = jdatetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    
    # Set column order
    columns_order = [
        'Symbol', 'Trade_Type', 'Time', 'Open', 'High', 'Low', 'Close', 'Final', 'Yesterday', 'Day_UL', 'Day_LL',
        ...
        'Buy-Price', 'Buy-Vol', 'Buy-No', 'Name', 'Market', 'Sector', 'EPS', 'Base_Vol', 'Share_No', 
